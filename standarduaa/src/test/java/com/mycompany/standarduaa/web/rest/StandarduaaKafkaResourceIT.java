...
import com.mycompany.standarduaa.config.KafkaProperties;
...
class StandarduaaKafkaResourceIT {
    ...
    void setup() {
        ...
        StandarduaaKafkaResource kafkaResource = new StandarduaaKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/standarduaa-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/standarduaa-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
