package com.mycompany.standardgateway.web.rest;

import com.mycompany.standardgateway.config.KafkaProperties;
...
class StandardgatewayKafkaResourceIT {
    ...
    void setup() {
        ...
        StandardgatewayKafkaResource kafkaResource = new StandardgatewayKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/standardgateway-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/standardgateway-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
